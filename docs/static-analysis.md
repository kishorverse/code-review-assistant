...
- **Line length.** PEP 8 lets a team agree on lines longer than 79 characters, and many projects are formatted at 88 or 100. Ruff uses the length the top-most `pyproject.toml` declares (`tool.ruff.lint.pycodestyle.max-line-length`, `tool.ruff.line-length` or `tool.black.line-length`), capped at 120 so a huge value cannot switch the check off. A project that configures Ruff or Black but sets no length has adopted that tool's own default of 88; only a project that configures neither is measured against PEP 8's 79. Scanning Margin's own code at 79 produced several hundred line-length findings for a project formatted at 100, and scanning requests and click at 79 produced 289 and 755 more (see the [evaluation](evaluation.md) §6).
...
- A secret in a test path (a `tests/`, `spec/`, `fixtures/` or `testdata/` directory, or a file named like a test) is reported one severity lower, and says so. Test suites are full of credentials written to be fake: across express and got, 78 of 81 high-severity secret findings were in tests. They are still reported, because real secrets do get committed in tests.
- **A finding cannot be marked a false positive in the source.** `# pragma: allowlist secret` is deliberately ignored, because in an upload it would let the uploader hide a secret from the scan and from redaction. That also means a repository has no way to tell Margin that its pinned git commit ids or vendored checksums are not secrets: Margin reports them, and the only escape today is `--exclude`, which is what this repository's own review workflow does for its evaluation harness. A suppression mechanism that is safe for untrusted uploads — a scan-level configuration rather than a comment in the code — is future work.
